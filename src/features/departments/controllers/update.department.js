...
export const updateDepartment = async (req, res) => {
  try {
    ...
    if (existing.deleted_at) {
      return res.status(409).json({ success: false, message: "Department is soft-deleted; restore it before updating" });
    }
    ...
    const noActiveChange = is_active === undefined || is_active === existing.is_active;
