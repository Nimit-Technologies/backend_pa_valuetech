  ...
  state.inflight = (async () => {
    try {
      ...
      if (
        ...
        console.warn("role counts drifted; reconciled from database", {
          memory: previous,
          database: counts,
        });
      }
...
export const getRoleCounts = async () => {
  if (state.counts) {
    if (Date.now() - state.loadedAt >= RECONCILE_MS) {
      reload().catch((error) =>
        console.error("role count reconcile failed:", error),
      );
    }
    return state.counts;
  }
  ...
  try {
    ...
    console.error("role count load failed:", error);
    return UNKNOWN;
  }
};
...
const contribution = (role) =>
  !role || role.deleted_at
    ? NONE
    : { total: 1, active: role.is_active ? 1 : 0 };

export const recordRoleTransition = (before, after) => {
  if (before === undefined || after === undefined) {
    invalidateRoleCounts();
    return;
  }
...
export const invalidateRoleCounts = () => {
  state.writeVersion += 1;
  state.counts = null;
  state.loadedAt = 0;
};
