...
export const updateRoleStatus = async (req, res) => {
    try {
        ...
        if (existing.deleted_at) {
            return res.status(409).json({ success: false, message: "Role is soft-deleted; restore it before changing status" });
        }

        const is_active = !existing.is_active;
        const role = await setRoleStatus(id, is_active);
        res.json({ success: true, message: `Role ${is_active ? "activated" : "deactivated"} successfully`, data: role });
    } catch (error) {
