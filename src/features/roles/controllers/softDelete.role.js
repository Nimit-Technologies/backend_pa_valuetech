...
export const softDeleteRole = async (req, res) => {
    try {
        ...
        if (existing.deleted_at) {
            return res.status(409).json({ success: false, message: "Role is already deleted" });
        }

        const role = await softDeleteRoleService(id);
        res.json({ success: true, message: "Role soft-deleted successfully", data: role });
    } catch (error) {
        console.error("softDeleteRole error:", error);
        res.status(500).json({ success: false, message: "Failed to soft-delete role" });
    }
};
