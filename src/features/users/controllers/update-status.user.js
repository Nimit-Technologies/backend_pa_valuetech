...
export const updateUserStatus = async (req, res) => {
  try {
    ...
    if (existing.deleted_at) {
      return res.status(409).json({ success: false, message: "User is soft-deleted; restore it before changing status" });
    }

    const is_active = !existing.is_active;
    const user = await setUserStatus(id, is_active);
    res.json({ success: true, message: `User ${is_active ? "activated" : "deactivated"} successfully`, data: user });
  } catch (error) {
