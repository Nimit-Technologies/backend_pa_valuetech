...
export const createUser = async (req, res) => {
  try {
    ...
    const {
      ...
      password, aadhaar_number, branch_id, department_id, role_id, address,
    } = parsed.data;
    ...
    if (respondIfInvalidParent(res, branch, { label: "Branch", action: "assign new users to it" })) return;
    if (respondIfInvalidParent(res, department, { label: "Department", action: "assign new users to it" })) return;
    if (respondIfInvalidParent(res, role, { label: "Role", action: "assign new users to it" })) return;

    const hashedPassword = await bcrypt.hash(password, CREDENTIALS.SALT_ROUNDS);

    const user = await createUserService({
      employee_id, first_name, last_name, email, phone,
      password: hashedPassword, aadhaar_number, branch_id, department_id, role_id, address,
    });
    ...
    if (error?.code === "P2002") {
      ...
      console.error(`createUser error: user with this ${UNIQUE_FIELD_LABELS[field] ?? field} already exists`);
