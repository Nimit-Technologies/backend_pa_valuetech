...
export const userSchema = z.object({
  ...
  aadhaar_number: z.string().length(12, "Aadhaar number must be 12 digits").regex(/^\d+$/, "Aadhaar number must contain only digits").transform((val) => val.trim()),
...
export const updateUserSchema = z.object({
  employee_id:   z.string().min(1).max(50).transform((val) => val.trim().toLowerCase()).optional(),
  ...
  aadhaar_number: z.string().length(12, "Aadhaar number must be 12 digits").regex(/^\d+$/, "Aadhaar number must contain only digits").transform((val) => val.trim()).optional(),
