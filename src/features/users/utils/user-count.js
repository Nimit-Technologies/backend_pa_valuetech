  ...
  state.inflight = (async () => {
    try {
      ...
      if (
        ...
        console.warn("user counts drifted; reconciled from database", {
          memory: previous,
          database: counts,
        });
      }
...
export const getUserCounts = async () => {
  if (state.counts) {
    if (Date.now() - state.loadedAt >= RECONCILE_MS) {
      reload().catch((error) =>
        console.error("user count reconcile failed:", error),
      );
    }
    return state.counts;
  }
  ...
  try {
    ...
    console.error("user count load failed:", error);
    return UNKNOWN;
  }
};
...
const contribution = (user) =>
  !user || user.deleted_at
    ? NONE
    : { total: 1, active: user.is_active ? 1 : 0 };

export const recordUserTransition = (before, after) => {
...
export const invalidateUserCounts = () => {
  state.writeVersion += 1;
  state.counts = null;
  state.loadedAt = 0;
};
